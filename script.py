...
def train_all(train_loader, test_loader, every_n = 4, num_epochs=max_epochs, random_features = False):
    ...
    for i, hidden in tqdm(enumerate(how_many_hiddens)):
        ...
        Path(f"models/{hidden}").mkdir(exist_ok=True, parents=True)
        plot(data=to_plot[0], labels=to_plot[1], x1='x1', x2='x2', y='y', title=f'Dataset with 2 classes; Model with {hidden} hidden neurons', save=True, figname=f'models/{hidden}/test.png', predictions=to_plot[2], show=False)
        torch.save(model.state_dict(), f"models/{hidden}/" + f"{hidden}_trained.pt")
...
def main():
    ...
    plot(data=my_data2, labels=my_labels2, x1='x1', x2='x2', y='y', title='Dataset with 2 classes', save=True, figname='plots/data.png', show=False)
    ...
    plt.legend()
    plt.savefig(f"plots/nn_experiment_scatter.png", dpi=300, bbox_inches='tight', pad_inches=0)
    plt.show()
    plt.plot(attempts, test_loss)
    plt.title(f"Trained NNs. {max_epochs} epochs. Test loss function", fontsize=20)
    plt.legend()
    plt.savefig("plots/nn_experiment_plot.png", dpi=300, bbox_inches='tight', pad_inches=0)
    plt.show()
